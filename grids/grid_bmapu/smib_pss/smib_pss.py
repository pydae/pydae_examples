...
if dae_file_mode == 'local':
    import smib_pss_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.smib_pss_cffi as jacs
if dae_file_mode == 'colab':
    import smib_pss_cffi as jacs
if dae_file_mode == 'testing':
    from pydae.temp import smib_pss_cffi as jacs
...
class model: 
    ...
    def __init__(self,matrices_folder='./build'): 
        ...
        self.params_values_list  = [100000000, 7.692307692307692, -38.46153846153846, 2e-06, 20000, 20000, 314.1592653589793, 200000000, 5, 8, 0.4, 1.8, 1.7, 0.3, 0.55, 0, 0.01, 0, 0, 100, 1e-06, 0.1, 0.1, 0.1, -10, 10, 10, 1, 1, 1, 1, 1, 0.1, 1000000000, 50, 0.001, 0, 0.001, 1e-06, 1e-06, 0, 0, 0.01] 
        self.inputs_ini_list = ['P_1', 'Q_1', 'P_2', 'Q_2', 'p_m_1', 'v_ref_1', 'alpha_2', 'v_ref_2', 'omega_ref_2', 'delta_ref_2', 'phi_2', 'rocov_2'] 
        self.inputs_ini_values_list  = [0, 0, 500000000, 0, 1.0, 1, 0, 1.0, 1.0, 0.0, 0.0, 0.0] 
        self.inputs_run_list = ['P_1', 'Q_1', 'P_2', 'Q_2', 'p_m_1', 'v_ref_1', 'alpha_2', 'v_ref_2', 'omega_ref_2', 'delta_ref_2', 'phi_2', 'rocov_2'] 
        self.inputs_run_values_list = [0, 0, 500000000, 0, 1.0, 1, 0, 1.0, 1.0, 0.0, 0.0, 0.0] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, f'./smib_pss_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz(f'{self.matrices_folder}/smib_pss_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './smib_pss_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './smib_pss_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz(f'{self.matrices_folder}/smib_pss_sp_jac_trap_num.npz')
        ...
        self.sp_Fu_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_Fu_run_num.npz')
        self.sp_Gu_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_Gu_run_num.npz')
        self.sp_Hx_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_Hx_run_num.npz')
        self.sp_Hy_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_Hy_run_num.npz')
        self.sp_Hu_run = sspa.load_npz(f'{self.matrices_folder}/smib_pss_Hu_run_num.npz')        
