...
if dae_file_mode == 'local':
    import sofc_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.sofc_cffi as jacs
if dae_file_mode == 'colab':
    import sofc_cffi as jacs
if dae_file_mode == 'testing':
    from pydae.temp import sofc_cffi as jacs
...
class model: 
    ...
    def __init__(self,matrices_folder='./build'): 
        ...
        self.N_z = 2 
        self.N_store = 100000 
        self.params_list = ['K_r', 'K_h2', 'K_h2o', 'K_o2', 'R_h01', 'U_opt', 'Tau_f', 'Tau_h2', 'Tau_h2o', 'Tau_o2', 'N0', 'E0', 'R_ohm'] 
        self.params_values_list  = [1.166e-06, 0.000843, 0.000281, 0.00252, 1.145, 0.85, 5, 26.1, 78.3, 2.91, 450, 1.18, 0.00032813] 
        self.inputs_ini_list = ['temp', 'i_dc_ref', 'di_dc'] 
        self.inputs_ini_values_list  = [1273, 0.0, 0.0] 
        self.inputs_run_list = ['temp', 'i_dc_ref', 'di_dc'] 
        self.inputs_run_values_list = [1273, 0.0, 0.0] 
        self.outputs_list = ['V_ernst', 'i_dc'] 
        self.x_list = ['q_h2', 'p_h2', 'p_h2o', 'p_o2', 'Di_dc'] 
        self.y_run_list = ['v_dc'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['v_dc'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, f'./sofc_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz(f'{self.matrices_folder}/sofc_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './sofc_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz(f'{self.matrices_folder}/sofc_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './sofc_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz(f'{self.matrices_folder}/sofc_sp_jac_trap_num.npz')
        ...
        #self.sp_Fu_run = sspa.load_npz(f'{self.matrices_folder}/sofc_Fu_run_num.npz')
        #self.sp_Gu_run = sspa.load_npz(f'{self.matrices_folder}/sofc_Gu_run_num.npz')
        #self.sp_Hx_run = sspa.load_npz(f'{self.matrices_folder}/sofc_Hx_run_num.npz')
        #self.sp_Hy_run = sspa.load_npz(f'{self.matrices_folder}/sofc_Hy_run_num.npz')
        #self.sp_Hu_run = sspa.load_npz(f'{self.matrices_folder}/sofc_Hu_run_num.npz')        
...
def sp_jac_ini_vectors():
    ...
    sp_jac_ini_ia = [0, 4, 0, 1, 4, 2, 4, 0, 3, 4, 4, 1, 2, 3, 4, 5]
    sp_jac_ini_ja = [0, 2, 5, 7, 10, 11, 16]
    sp_jac_ini_nia = 6
    sp_jac_ini_nja = 6
...
def sp_jac_run_vectors():
    ...
    sp_jac_run_ia = [0, 4, 0, 1, 4, 2, 4, 0, 3, 4, 4, 1, 2, 3, 4, 5]
    sp_jac_run_ja = [0, 2, 5, 7, 10, 11, 16]
    sp_jac_run_nia = 6
    sp_jac_run_nja = 6
...
def sp_jac_trap_vectors():
    ...
    sp_jac_trap_ia = [0, 4, 0, 1, 4, 2, 4, 0, 3, 4, 4, 1, 2, 3, 4, 5]
    sp_jac_trap_ja = [0, 2, 5, 7, 10, 11, 16]
    sp_jac_trap_nia = 6
    sp_jac_trap_nja = 6
