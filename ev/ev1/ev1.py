...
if dae_file_mode == 'local':
    import ev1_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.ev1_cffi as jacs
...
class model: 
    ...
    def __init__(self): 
        ...
        self.N_z = 8 
        self.N_store = 100000 
        self.params_list = ['R_w', 'G', 'M', 'K_w', 'C_rr', 'Rho', 'S_f', 'C_x', 'K_sign', 'R_a', 'L_a', 'R_f', 'L_f', 'L_af', 'N_pp', 'K_p_a', 'K_i_a', 'C_b', 'R_b', 'K_p_b', 'K_i_b', 'E_b0', 'E_b1'] 
        self.params_values_list  = [0.316, 9.81, 1200.0, 1.0, 0.03, 1.225, 2.13, 0.32, 100, 0.06727, 0.001882, 30.72, 3.166, 0.2641, 1.0, 1.0, 1.0, 100000.0, 0.1, 1e-05, 1e-05, 300, 320] 
        self.inputs_ini_list = ['tau_r_ref', 'beta', 'v_f', 'soc_ref'] 
        self.inputs_ini_values_list  = [0.0, 0.0, 220.0, 0.5] 
        self.inputs_run_list = ['tau_r_ref', 'beta', 'v_f', 'soc_ref'] 
        self.inputs_run_values_list = [0.0, 0.0, 220.0, 0.5] 
        self.outputs_list = ['f_w', 'f_d', 'f_r', 'tau_r', 'p_r', 'nu_kmh', 'v_b', 'i_b'] 
        self.x_list = ['nu', 'i_a', 'i_f', 'xi_a', 'soc', 'xi_b'] 
        self.y_run_list = ['snu', 'omega_r', 'eta_a', 'i_ch', 'v_b'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['snu', 'omega_r', 'eta_a', 'i_ch', 'v_b'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './ev1_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz('./ev1_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './ev1_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz('./ev1_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './ev1_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz('./ev1_sp_jac_trap_num.npz')
        ...
        #self.sp_Fu_run = sspa.load_npz('./ev1_Fu_run_num.npz')
        #self.sp_Gu_run = sspa.load_npz('./ev1_Gu_run_num.npz')
        #self.sp_Hx_run = sspa.load_npz('./ev1_Hx_run_num.npz')
        #self.sp_Hy_run = sspa.load_npz('./ev1_Hy_run_num.npz')
        #self.sp_Hu_run = sspa.load_npz('./ev1_Hu_run_num.npz')        
...
def sp_jac_ini_vectors():
    ...
    sp_jac_ini_ia = [0, 1, 2, 6, 1, 2, 7, 8, 10, 2, 1, 2, 1, 8, 9, 4, 0, 6, 0, 7, 1, 2, 3, 8, 4, 5, 9, 1, 4, 8, 10]
    sp_jac_ini_ja = [0, 4, 9, 10, 12, 15, 16, 18, 20, 24, 27, 31]
    sp_jac_ini_nia = 11
    sp_jac_ini_nja = 11
...
def sp_jac_run_vectors():
    ...
    sp_jac_run_ia = [0, 1, 2, 6, 1, 2, 7, 8, 10, 2, 1, 2, 1, 8, 9, 4, 0, 6, 0, 7, 1, 2, 3, 8, 4, 5, 9, 1, 4, 8, 10]
    sp_jac_run_ja = [0, 4, 9, 10, 12, 15, 16, 18, 20, 24, 27, 31]
    sp_jac_run_nia = 11
    sp_jac_run_nja = 11
...
def sp_jac_trap_vectors():
    ...
    sp_jac_trap_ia = [0, 1, 2, 6, 1, 2, 7, 8, 10, 2, 1, 2, 3, 1, 4, 8, 9, 4, 5, 0, 6, 0, 7, 1, 2, 3, 8, 4, 5, 9, 1, 4, 8, 10]
    sp_jac_trap_ja = [0, 4, 9, 10, 13, 17, 19, 21, 23, 27, 30, 34]
    sp_jac_trap_nia = 11
    sp_jac_trap_nja = 11
