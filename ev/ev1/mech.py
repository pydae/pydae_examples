...
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.mech_cffi as jacs
...
class model: 
    ...
    def __init__(self): 
        ...
        self.N_z = 6 
        self.N_store = 100000 
        self.params_list = ['R_w', 'G', 'M', 'K_w', 'C_rr', 'Rho', 'S_f', 'C_x', 'K_sign', 'R_a', 'L_a', 'R_f', 'L_f', 'L_af', 'N_pp', 'K_p_a', 'K_i_a'] 
        self.params_values_list  = [0.316, 9.81, 1200.0, 1.0, 0.03, 1.225, 2.13, 0.32, 100, 0.06727, 0.001882, 30.72, 3.166, 0.2641, 1.0, 1.0, 1.0] 
        self.inputs_ini_list = ['tau_r_ref', 'beta', 'v_f'] 
        self.inputs_ini_values_list  = [0.0, 0.0, 220.0] 
        self.inputs_run_list = ['tau_r_ref', 'beta', 'v_f'] 
        self.inputs_run_values_list = [0.0, 0.0, 220.0] 
        self.outputs_list = ['f_w', 'f_d', 'f_r', 'tau_r', 'p_r', 'nu_kmh'] 
        self.x_list = ['nu', 'i_a', 'i_f', 'xi_a'] 
        self.y_run_list = ['snu', 'omega_r', 'v_a'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['snu', 'omega_r', 'v_a'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './mech_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz('./mech_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './mech_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz('./mech_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './mech_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz('./mech_sp_jac_trap_num.npz')
        ...
        #self.sp_Fu_run = sspa.load_npz('./mech_Fu_run_num.npz')
        #self.sp_Gu_run = sspa.load_npz('./mech_Gu_run_num.npz')
        #self.sp_Hx_run = sspa.load_npz('./mech_Hx_run_num.npz')
        #self.sp_Hy_run = sspa.load_npz('./mech_Hy_run_num.npz')
        #self.sp_Hu_run = sspa.load_npz('./mech_Hu_run_num.npz')        
...
def sp_jac_ini_vectors():
    ...
    sp_jac_ini_ia = [0, 1, 2, 4, 1, 2, 5, 6, 2, 1, 2, 0, 4, 0, 5, 1, 2, 3, 6]
    sp_jac_ini_ja = [0, 4, 8, 9, 11, 13, 15, 19]
    sp_jac_ini_nia = 7
    sp_jac_ini_nja = 7
...
def sp_jac_run_vectors():
    ...
    sp_jac_run_ia = [0, 1, 2, 4, 1, 2, 5, 6, 2, 1, 2, 0, 4, 0, 5, 1, 2, 3, 6]
    sp_jac_run_ja = [0, 4, 8, 9, 11, 13, 15, 19]
    sp_jac_run_nia = 7
    sp_jac_run_nja = 7
...
def sp_jac_trap_vectors():
    ...
    sp_jac_trap_ia = [0, 1, 2, 4, 1, 2, 5, 6, 2, 1, 2, 3, 0, 4, 0, 5, 1, 2, 3, 6]
    sp_jac_trap_ja = [0, 4, 8, 9, 12, 14, 16, 20]
    sp_jac_trap_nia = 7
    sp_jac_trap_nja = 7
