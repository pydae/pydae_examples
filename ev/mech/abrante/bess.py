...
if dae_file_mode == 'local':
    import bess_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.bess_cffi as jacs
...
class model: 
    ...
    def __init__(self): 
        ...
        self.N_z = 1 
        self.N_store = 100000 
        self.params_list = ['Cp', 'Np', 'Ns', 'K_charger'] 
        self.params_values_list  = [66.2, 2, 96, 1e-06] 
        self.inputs_ini_list = ['Ibat', 'SOC0', 'u_dummy'] 
        self.inputs_ini_values_list  = [0.0, 0.5, 1.0] 
        self.inputs_run_list = ['Ibat', 'SOC0', 'u_dummy'] 
        self.inputs_run_values_list = [0.0, 0.5, 1.0] 
        self.outputs_list = ['Vbat_calc'] 
        self.x_list = ['SOC', 'V1', 'V2'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './bess_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz('./bess_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './bess_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz('./bess_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './bess_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz('./bess_sp_jac_trap_num.npz')
        ...
        #self.sp_Fu_run = sspa.load_npz('./bess_Fu_run_num.npz')
        #self.sp_Gu_run = sspa.load_npz('./bess_Gu_run_num.npz')
        #self.sp_Hx_run = sspa.load_npz('./bess_Hx_run_num.npz')
        #self.sp_Hy_run = sspa.load_npz('./bess_Hy_run_num.npz')
        #self.sp_Hu_run = sspa.load_npz('./bess_Hu_run_num.npz')        
...
def sp_jac_run_vectors():
    ...
    sp_jac_run_ia = [0, 0, 1, 0, 2, 3]
    sp_jac_run_ja = [0, 1, 3, 5, 6]
    sp_jac_run_nia = 4
    sp_jac_run_nja = 4
...
def sp_jac_trap_vectors():
    ...
    sp_jac_trap_ia = [0, 0, 1, 0, 2, 3]
    sp_jac_trap_ja = [0, 1, 3, 5, 6]
    sp_jac_trap_nia = 4
    sp_jac_trap_nja = 4
